...
public final class BrawlGameState extends Observable
{
   ...
   public void startNewGame()
   {
      ...
      this.players.put(PlayerID.one, new Player(new BrawlDeck((BrawlCharacter) SettingsManager.
          getSettingValue(Setting.P1Character))));
      this.players.put(PlayerID.two, new Player(new BrawlDeck((BrawlCharacter) SettingsManager.
          getSettingValue(Setting.P2Character))));

      // Initialize the game board - there are 3 lanes in any game of brawl
      // the board starts with two bases
      this.lanes = new ArrayList<Lane>();
      // Per the brawl rules, add one bese for each character to start out with
      lanes.add(new Lane(new BaseCard((BrawlCharacter) SettingsManager.getSettingValue(
          Setting.P1Character))));
      lanes.add(new Lane(new BaseCard((BrawlCharacter) SettingsManager.getSettingValue(
          Setting.P2Character))));
   }
